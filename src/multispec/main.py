...
def unpack_band_dipoles(band_params: np.ndarray, index: int, symmetry_mode: str) -> tuple[float, float, float]:
...
def run_global_sh_fit(bands_dict: dict, flat_real_temps: list[float], flat_fields: list[float], flat_nominal_temps: list[float], mol_config: dict) -> None:
    ...
    curve_data_rows = plot_magnetization_fit_grid(
        bands_dict, flat_real_temps, flat_fields, flat_nominal_temps,
        all_plot_params, mag_basis, smooth_fields, out_dir, plot_reduced_mag=plot_reduced_mag
    )
    plot_isofield_summary(
        bands_dict, flat_real_temps, flat_fields, all_plot_params,
        iso_basis, smooth_temps, target_field, out_dir
    )
    save_sh_results_to_csv(sh_csv_data, D_fit, E_fit, g_tensor, out_dir)
    save_simulated_curves_to_csv(curve_data_rows, out_dir)

def run_standalone_sh(config: dict, selected_fields: list[float] | None = None, selected_temps: list[float] | None = None) -> None:
    if 'sh' not in config:
