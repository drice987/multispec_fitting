...
from .constants import k_B, mu_b
...
def build_sh_parameters(mol_config: dict, symmetry_mode: str) -> dict[str, FitParameter]:
    ...
    if e_vary:
        ...
        sh_params["eta"] = FitParameter(
            "eta", {"value": eta_init, "min": 0.0, "max": 1 / 3.0, "vary": True}
        )
...
def calculate_polarizations(
    Mxy: float, Myz: float, Mxz: float, eps: float = 1e-12
) -> PolarizationResult:
...
def generate_simulation_grids(
    engine: "SpinHamiltonian", unique_temps: np.ndarray, max_field: float
) -> tuple[np.ndarray, dict, np.ndarray, list]:
    ...
    iso_basis = [
        engine.get_mcd_components(max_field, t, n_theta=30, n_phi=30) for t in smooth_temps
    ]
...
class SpinHamiltonian:
    ...
    def __init__(self, S: float, D: float, E: float, g: float | list[float]) -> None:
        ...
        self.H_zfs = D * (self.Sz @ self.Sz - (S_sq / 3.0) * identity) + E * (
    ...
    def solve(self, B_vector: list[float]) -> tuple[np.ndarray, np.ndarray]:
        ...
        Bx, By, Bz = B_vector
        H_zeeman = self.mu_b * (
            self.gx * Bx * self.Sx + self.gy * By * self.Sy + self.gz * Bz * self.Sz
        )
    ...
    def get_mcd_components(
        self, B_mag: float, temp: float, n_theta: int = 30, n_phi: int = 30
        ...
        exp_Sx = np.diagonal(
            wavefunctions.conj().swapaxes(-1, -2) @ self.Sx @ wavefunctions, axis1=-2, axis2=-1
        ).real
        exp_Sy = np.diagonal(
            wavefunctions.conj().swapaxes(-1, -2) @ self.Sy @ wavefunctions, axis1=-2, axis2=-1
        ).real
        exp_Sz = np.diagonal(
            wavefunctions.conj().swapaxes(-1, -2) @ self.Sz @ wavefunctions, axis1=-2, axis2=-1
        ).real
...
class MagnetizationFitter:
    ...
    def run_fit(self, method: str = "least_squares") -> Any:
        ...
        if method not in supported_methods:
            raise ValueError(
                f"Invalid optimization method: '{method}'. Supported methods are: {', '.join(supported_methods)}"
            )
