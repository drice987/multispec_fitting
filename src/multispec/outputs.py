...
def plot_results(
    dataset: DataSet, bands: list[SpectralBand], namespace: dict, filename: str = "fit_results.png"
) -> None:
...
def save_results_to_toml(filename: str, dataset: DataSet, bands: list[SpectralBand]) -> None:
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        if dataset.is_vtvh:
            ...
            temps_formatted = ", ".join(
                [f'"{t}"' if isinstance(t, str) else str(t) for t in dataset.toml_temperatures]
            )
...
def save_results_to_csv(
    ...
    namespace: dict,
    param_filename: str = "output_parameters.csv",
    amp_filename: str = "output_amplitudes.csv",
) -> None:
...
def save_sh_results_to_csv(
    sh_data: list,
    D: float,
    E: float,
    g: float | list[float],
    out_dir: Path,
    filename: str = "sh_fit_parameters.csv",
) -> None:
...
def save_spectra_to_csv(
    dataset: DataSet,
    bands: list[SpectralBand],
    namespace: dict,
    filename: str = "output_spectra.csv",
) -> None:
    x_axis = dataset.get_x()
    conditions = dataset.conditions
    band_matrices = [band.evaluate(x_axis, namespace) for band in bands]
...
def plot_sh_curves(
    ...
    mag_basis: dict,
    smooth_fields: np.ndarray,
    curve_data_rows: list,
    flat_nominal_temps: list[float],
    plot_reduced_mag: bool = True,
) -> None:
...
def plot_isofield_summary(
    bands_dict: dict,
    flat_temps: list[float],
    flat_fields: list[float],
    all_plot_params: dict,
    iso_basis: list,
    smooth_temps: np.ndarray,
    target_field: float,
    out_dir: Path,
) -> None:
...
def plot_magnetization_fit_grid(
    bands_dict: dict,
    flat_real_temps: list[float],
    flat_fields: list[float],
    flat_nominal_temps: list[float],
...
def save_simulated_curves_to_csv(
    curve_data_rows: list, out_dir: Path, filename: str = "sh_simulated_curves.csv"
) -> None:
